...
Changing the upper range of the time delay maximum range, can however change the reconstruction and the fitting statistics. The upper range of this parameter which is called as 'dt\_max'in the program, used in this run of the code, can be found in the naming nomenclature of the folder TD\_$20$, meaning the uppper range of the time delay max is $20$ day (the default lower limit is $0$). The folder name also shows the number of parameter used which is $NP=8$. 
...
\fi
Table in the next page, showing the reconstruction statistics, \textbf{as a function of the node space parameter($1^{st}$ column)} for a given dt\_max(which in this case is $20$ for $NP=8$ parameters.
\includepdf[pages=-]{Reconstruction-TD_20_NP_8.pdf}\label{T}

\includepdf[pages={1-},nup=1x4,scale=1.0]{Reconstruction-TD_20_NP_8_plots.pdf}

%\includepdf[pages=-]{Reconstruction-TD_20_NP_8_plots.pdf}
\includepdf[pages=-]{corner-TD_20_NP_8.pdf}
\end{document}
